...
module.exports = router 
    ...
    .put('/me', ensureRole(['donor']), (req, res, next) => {
        const myToken = req.get('Authorization');
        return tokenService.verify(myToken)
            .then(( body ) => {
                return Donation.findOneAndUpdate({ donor: body.id }, req.body, { new: true })
                    .populate({ path: 'dropSite', select: 'name'});
            })  
            .then(donation => {
                if (!donation) {
    ...
    .delete('/me/:id', ensureRole(['donor']), (req, res, next) => {
        ...
        return tokenService.verify(myToken)
            .then(( body ) => {
                return Donation.findOneAndRemove({ _id: req.params.id, donor: body.id });
            })  
            .then(donation => {
                if (!donation) {
                    next({code: 404, error:'invalid user id'});
